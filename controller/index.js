...
var showContent = function(req, res, next) {
  ...
  return; // Disable Auth Required feature
  authByCookie(req, function(err, user) {
    if(err) {
      res.cookie("scorll.login.content-id", res.local('contentId'));
      res.cookie("scorll.login.content-alias", res.local('contentAlias'));
      res.redirect('/login');
      return;
    }
    app.contentSet.findById(res.local('contentId'), function(err, content) {
